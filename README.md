...
경기 결과는 `R16_RESULTS`(매치 인덱스 → 승자 코드)에 추가한다.

국기는 팀당 1개만 그린다(프런티어 방식): 각 팀은 이긴 만큼 들어간 현재 슬롯에만
국기가 놓이고, 진 팀은 탈락한 그 슬롯에 무채색·축소 국기로 남는다(사선 없음).
비워진 슬롯은 점으로 표시되어, 결과가 쌓일수록 컬러 국기가 중앙 트로피 쪽으로
조여든다. 승자가 정해진 대진은 예상(점선)에서 확정(실선)으로 바뀐다.
